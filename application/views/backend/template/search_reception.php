...
<?= form_open('reception/index', $attributes); ?>
      ...
      <div class="col-lg-12">
        <select id="search_status" class="form-control" name="search_status" placeholder="เขตพื้นที่">
          ...
          <?php foreach ($search_statuss as $status): ?>
            <?php if($status['request_status'] != null): ?>
            <option value="<?=$status['request_status']?>" <?php if($status['request_status'] == $search_status) { echo "selected"; } ?>>
              <?=$status_text[$status['request_status']]?>
            </option>
            <?php endif; ?>

            <?php if($status['ticket_status'] != null): ?>
            <option value="<?=$status['ticket_status']?>">
